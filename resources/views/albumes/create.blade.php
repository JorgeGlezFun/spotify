<x-app-layout>
    <div class="w-1/2 mx-auto">
        <form method="POST" action="{{ route('albumes.store') }}">
            ...
            <div>
                <x-input-label for="temas" :value="'Tema(s)'" />
                @foreach ($temas as $tema)
                <div class="flex items-center mb-4">
                    <input name="temas[]" id="tema{{ $tema->id }}" type="checkbox" value="{{$tema->id}}" class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600">
                    <label for="tema{{ $tema->id }}" class="ms-2 text-sm font-medium text-">{{$tema->titulo}}</label>
                </div>
                @endforeach
            </div>
